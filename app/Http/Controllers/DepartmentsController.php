...
class DepartmentsController extends Controller
{
    ...
    public function indexData()
    {
        ...
        return Datatables::of($departments)
            ...
            ->addColumn('edit', '
                <a href="{{ route(\'departments.edit\', $external_id) }}" class="edit btn btn-primary btn-sm" >'  . __('Edit') . '</a>')
            ->addColumn('delete', '
                <form action="{{ route(\'departments.destroy\', $external_id) }}" method="POST">
            <input type="hidden" name="_method" value="DELETE">
            {{csrf_field()}}
            <input type="submit" name="submit" value="' . __('Delete') . '" class="btn btn-danger btn-sm" onClick="return confirm(\'Are you sure?\')"">
            </form>')
